...
// Group page uses static background in this implementation; remove video import
...
const GroupChatPage = memo(() => {
  ...
  }, [groupId, groupState.selectedGroupId]); // 移除 groupActions 依赖
  ...
  }, [groupId, user]); // 移除 characters 和 performanceActions 依赖

  // 单独处理角色数据更新时的群组成员更新
  useEffect(() => {
    if (currentGroup && characters.length > 0) {
      const memberCharacters = characters.filter((char: Character) => 
        currentGroup.groupMemberIds.includes(char.id)
      );
      setGroupMembers(memberCharacters);
    }
  ...
  }, [groupId, user, isSendingMessage]); // 移除 performanceActions 依赖
  ...
  const handleGroupUpdated = useCallback(async () => {
    ...
    try {
      ...
      if (group) {
        ...
        // Update group members - 使用当前的characters状态
        const memberCharacters = characters.filter((char: Character) => 
          group.groupMemberIds.includes(char.id)
        );
        setGroupMembers(memberCharacters);
  ...
  }, [backgroundState.isVideoReady, backgroundState.videoError]); // 移除 backgroundActions 依赖
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.backgroundContainer}>
        <ImageBackground
          source={backgroundActions.getGroupBackgroundImage(selectedGroup)}
        ...
        <>
          ...
          <View style={styles.mainContent}>
            <GroupInterface
              ...
              onGroupDisbanded={handleGroupDisbanded}
              topBarHeight={68} // 优化后的TopBar高度：navbarHeight(20) + topBarContentHeight(AVATAR_SIZE+16)
            />
          </View>
        </>
      )}
